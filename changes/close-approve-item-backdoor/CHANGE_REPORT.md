...
| 文件同步 | `docs/api_contract.md`、`CLAUDE.md`、`docs/notes.md` | AC8 |

程式與文件共 **7 個檔案**,全部落在 Plan 批准的路徑範圍內。
(此處刻意不寫累計行數:`main..HEAD` 是會移動的目標,寫進文件就注定過期——
這是上一個變更 `ingest-concurrency-guard` 第三輪審查學到的。)
...
- **L4 — 守衛測試曾經是假的(審查 M1),已修;但仍有殘留限制。**
  ...
  **殘留限制**:刪掉該測試再加回端點仍然可行。程式碼註解、`CLAUDE.md`、`README.md`
  各留了警告(再加回來就要把四道守衛一起加回來),但那是溝通,不是強制。
