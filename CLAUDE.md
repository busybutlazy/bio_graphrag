...
**Curation governance flow** (`app/curation/service.py`): propose a *statement* (`create_group`, or LLM staging — both write `curation_items` sharing one `group_id`) → the group review queue → `approve_group` writes the whole group into Neo4j as `approved` / `reject_group` / `record_gap`, plus `merge_nodes` / `delete_node`(→`deprecated`) for already-approved knowledge. Every mutation appends to `graph_change_logs` with actor + action + reason. Node/edge types are validated against whitelists (`VALID_NODE_TYPES` / `VALID_RELATIONSHIP_TYPES` in `ingestion/pipeline/normalize_concepts.py`) at *propose* time — critical because approval interpolates the type into a Cypher label, so an unvalidated type would be an injection vector.

**The group is the unit of review: no proposal reaches the graph one item at a time.** (Curator edits to *already-approved* knowledge — `POST /admin/graph/{merge-nodes,delete-node,delete-edge}` — are still per-item and deliberately outside the gates; they are not a proposal path.) `create_item` / `approve_item` / `reject_item` and their `POST /admin/curation/items*` endpoints were removed (`changes/close-approve-item-backdoor`): the former staged rows with no `group_id`, invisible to the review queue, and the latter wrote them into Neo4j behind nothing but a `status == 'proposed'` check — no Schema gate, no back-translation, no deprecated-resurrection or dangling-edge guard, all of which `approve_group` enforces. Re-adding a single-item approval means re-adding those guards with it.
